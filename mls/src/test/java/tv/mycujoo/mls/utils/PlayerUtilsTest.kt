...
class PlayerUtilsTest {
    ...
    companion object {
        fun eventWithRawVideo(): EventEntity {
            return event(listOf(Stream("stream_id", Long.MAX_VALUE.toString(), "sample_url", null)))
        }
        ...
        fun eventWithBothWidevineAndRawVideo(): EventEntity {
            val widevine = Widevine("sample_url", "license_url")
            return event(listOf(Stream("stream_id", Long.MAX_VALUE.toString(), "license_url", widevine)))
        }
